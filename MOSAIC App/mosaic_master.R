...
calc_param_echo_joint_constr <- function(current_gene, timen, resol, num_reps, start_type = "orig", res, harm_cut, over_cut){
...
# outputs:
#  list which contains: model name, fit model object, parameters, joint p value, rna p value, protein p value, data frame of parameters for output, ac coefficient category for rna and protein
calc_param_echo_lin_joint_constr <- function(current_gene, timen, resol, num_reps, start_type = "orig", res, harm_cut, over_cut){
  gene_n <- as.character(genes_rna[current_gene,1]) # gene name
...
calc_param_echo_joint <- function(current_gene, timen, resol, num_reps, start_type = "orig", harm_cut, over_cut){
  gene_n <- as.character(genes_rna[current_gene,1]) # gene name
...
# outputs:
#  list which contains: model name, fit model object, and parameters
calc_param_echo_lin_joint <- function(current_gene, timen, resol, num_reps, start_type = "orig", harm_cut, over_cut){
  gene_n <- as.character(genes_rna[current_gene,1]) # gene name
...
calc_param_echo <- function(current_gene, timen, resol, num_reps, genes, avg_genes, harm_cut, over_cut){
...
multi_pipeline <- function(current_gene, timen, resol, num_reps, final_df_row, genes_rna, genes_pro, rem_unexpr_combine, harm_cut = .03, over_cut = .15){
  # preallocating
  ...
  if (length(send_joint) > 0){ # if we have a joint model
    ...
    if ("echo" %in% c(aic_rna$best_mod_orig[1], aic_pro$best_mod_orig[1])){
      ...
      ej_mod <- calc_param_echo_joint(current_gene, timen, resol, num_reps, "orig", harm_cut, over_cut)
      res <- ej_mod$param #extract parameter estimates
      
      # final fit: echo joint model with slack for protein
      ejc_mod <- calc_param_echo_joint_constr(current_gene, timen, resol, num_reps, "orig", res, harm_cut, over_cut)
    } else {
      # get one choice for starting points, using a completely joint model
      # echo lin completely joint model
      ej_mod <- calc_param_echo_lin_joint(current_gene, timen, resol, num_reps, "orig", harm_cut, over_cut)
      res <- ej_mod$param #extract parameter estimates
      
      # final fit: echo linear joint model with slack for protein
      ejc_mod <- calc_param_echo_lin_joint_constr(current_gene, timen, resol, num_reps, "orig", res, harm_cut, over_cut)
    }
